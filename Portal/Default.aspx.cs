...
namespace Portal
{
	public partial class MyDefault : System.Web.UI.Page
	{
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (Request.QueryString["ID"] != null && Request.QueryString["ID"].ToLower() == "reettemplatecatch")
			{
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "Site.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "ie9.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "ie8.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "ie6.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "chrome.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "firefox.css");
				Tools.Tools.DeleteFile(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "Mob.css");
				Tools.Template.CadrArray.Clear();
				Tools.Template.TemplateArray.Clear();
			}
			else if (Request.QueryString["ID"] != null && Request.QueryString["ID"].ToLower() == "en")
			{
				HttpCookie mylang = new HttpCookie("MyLanguage");
				mylang.Value = "en-us";
				mylang.Expires = DateTime.Now.AddYears(1);
			...
			else if (Request.QueryString["ID"] != null && Request.QueryString["ID"].ToLower() == "de")
			{
				HttpCookie mylang = new HttpCookie("MyLanguage");
				mylang.Value = "de-gr";
				mylang.Expires = DateTime.Now.AddYears(1);
			...
			else if (Request.QueryString["ID"] != null && Request.QueryString["ID"].ToLower() == "fa")
			{
				HttpCookie mylang = new HttpCookie("MyLanguage");
				mylang.Value = "fa-ir";
				mylang.Expires = DateTime.Now.AddYears(1);
		...
		//		Page.ClientScript.RegisterClientScriptBlock(Page.GetType(), "", string.Format("alert('{0}');", "نسخه جدید سايت از مرورگر شما پشتیبانی نمی کند شما می توانید از فایرفاکس یا مروگر اکپلورر ورژن 6 به بعد استفاده نمایید"), true);
			...
			if (!File.Exists(DAL.CheckData.GetFilesRoot(true) + "\\" + Tools.Tools.LangSTR + "Site.css"))
				Tools.Tools.GreateCSSView();

			string MyDefaultType = Tools.Tools.GetSetting(368, ""); 
			if (MyDefaultType == "")
			...
			if (Tools.Tools.ConvertToInt32(MyDefaultType) < 0)
			{
				...
				while (CTemplate != "")
				{
					...
					CTemplate = CTemplate.Substring(stind);
					int enind = CTemplate.IndexOf("#",1);
					if (enind != -1)
					{
						if (CTemplate.Substring(0, enind).IndexOf(".ascx") != -1)
						{
							string ASCXName = CTemplate.Substring(0, enind).Replace("#", "");
							if (File.Exists(Server.MapPath("def")+"\\" + ASCXName))
							{
				...
				if (MyRead.Read())
				{
					if (MyDefaultType != "3")
					{
						...
						if (Tools.MyCL.MGInt(MyRead, 3) != 0)//d23
						{
							...
							//else if (MyDefType == "2")
							//  DL23a.Controls.Add(LoadControl("Def/" + Global.ASCXPages[Tools.MyCL.MGInt(MyRead, 3)]));
						}
        ...
        {
            Global.ASCXPages = new string[200];
