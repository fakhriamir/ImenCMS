<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true" CodeBehind="SiteMap.aspx.cs" Inherits="Portal.Admin.SiteMap" %>
	...
	<table dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" class="TableColor" align="center" id="Table1" cellspacing="0" cellpadding="1" width="80%">
		<tr>
			<td colspan="2" class="RowHead">
				<%=GetGlobalResourceObject("resource", "SiteMapMng")%>
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			<td>
				<%=GetGlobalResourceObject("resource", "BranchName")%>
			</td>
			<td>
		...
		<tr>
			<td>
				<%=GetGlobalResourceObject("resource", "TopBranch")%>
			</td>
			<td>
		...
		<tr>
			...
			<td>
				<asp:TextBox ID="AddressTB" runat="server" MaxLength="128" />*
			</td>
		</tr>
		...
		<asp:Repeater runat="server" ID="ViewDR" OnItemCommand="ViewDR_ItemCommand">
			<ItemTemplate>
				<tr class="<%# Container.ItemIndex % 2 == 0 ? "RowItem" : "RowAlter" %>">
					<td align="center">
						&nbsp;<%# Eval("sitemapID").ToString().Trim() %>
					</td>
					<td align="center">
						&nbsp;<%# Eval("Name").ToString().Trim() %>
					</td>
					<td align="center">
						<asp:ImageButton ImageUrl="Imgs/edit.gif" Width="18px" Height="18px" CommandArgument='<%# Eval("sitemapID").ToString().Trim() %>'
					...
					<td align="center">
						<asp:ImageButton OnClientClick="return confirm_delete();" ImageUrl="Imgs/zarb.gif"
							Width="16px" Height="16px" CommandArgument='<%# Eval("sitemapID").ToString().Trim() %>'
