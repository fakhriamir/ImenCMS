<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true"
	CodeBehind="ProductSubject.aspx.cs" Inherits="Portal.Admin.ProductSubject" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Body" runat="server">
	<table dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" class="TableColor" align="center" id="Table1" cellspacing="1" cellpadding="1"
		...
		<tr>
			<td>
				<%=GetGlobalResourceObject("resource", "SaleFile")%>
			</td>
			<td>
				<asp:DropDownList ID="FilesSalesDL" runat="server">
					<asp:ListItem Selected="True" Value="0" Text="فروش محصول معمولی"></asp:ListItem>
					<asp:ListItem Value="1"  Text="فروش فایل" ></asp:ListItem>
					<asp:ListItem Value="2"  Text="نوع کاربری" ></asp:ListItem>
				</asp:DropDownList>
			</td>
		</tr>
   ...
   {%>
	<table dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" id="My" align="center" cellspacing="1" cellpadding="2" width="100%"
		...
		<tr class="RowHead">
			...
			<td>
				<%=GetGlobalResourceObject("resource", "PrdComponents")%>
			</td>
			<td>
				تعداد محصول
			</td>
		...
		<asp:Repeater runat="server" ID="ViewDR" OnItemCommand="ViewDR_ItemCommand">
			<ItemTemplate>
				<tr class="<%# Container.ItemIndex % 2 == 0 ? "RowItem" : "RowAlter" %>">
					<td align="center">
						&nbsp;<%# Eval("ProductSubjectID").ToString().Trim()%>
					</td>
					<td align="center">
						&nbsp;<%# Eval("Name").ToString().Trim() %>
					</td>
					<td>
						<a href="ProductSubjectItem.aspx?ID=<%# Eval("ProductSubjectID").ToString().Trim()%>">
							<%=GetGlobalResourceObject("resource", "PrdComponents")%></a>
					</td>
					<td>
<%#GetProductSubjectCNT(Eval("ProductSubjectID").ToString().Trim())%>
					</td>
					<td align="center">
						<asp:ImageButton ImageUrl="Imgs/edit.gif" Width="18px" Height="18px" CommandArgument='<%# Eval("ProductSubjectID").ToString().Trim() %>'
					...
					<td align="center">
						<asp:ImageButton OnClientClick="return confirm_delete();" ImageUrl="Imgs/zarb.gif"
							Width="16px" Height="16px" CommandArgument='<%# Eval("ProductSubjectID").ToString().Trim() %>'
