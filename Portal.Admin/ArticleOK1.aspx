<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true" CodeBehind="ArticleOK1.aspx.cs" Inherits="Portal.Admin.ArticleOK1" %>
			...
			<table class="<%# Container.ItemIndex % 2 == 0 ? "RowItem" : "RowAlter" %>" dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" id="Table1" cellspacing="0" cellpadding="1" width="80%" border="1">
				...
				<tr>
					<td>
						...
						<asp:Button Text="<%$ resources: resource,ArticleOK %>" CommandArgument='<%# Eval("articleid") %>' CommandName="EDIT"
							ID="EditBTN" ToolTip="<%$ resources: resource,ArticleOK %>" AlternateText="<%$ resources: resource,ArticleOK %>" runat="server" />
						<asp:ImageButton OnClientClick="return confirm_delete();" ImageUrl="Imgs/zarb.gif"
							Width="16px" Height="16px" CommandArgument='<%# Eval("articleid") %>' CommandName="DEL"
							ID="DelBTN" ToolTip="<%$ resources: resource, Del %>" AlternateText="<%$ resources: resource, Del %>" runat="server" />
						<input style="width: 80px" type="button" onclick="SelectPrepMsg('/Messagesend.aspx?UserID=<%#Eval("UserID")%>&Content=<%# HttpUtility.UrlEncode("مقاله:"+ Eval("Title").ToString().Trim())%>',520,240);"
							value="<%=GetGlobalResourceObject("resource", "Reference")%>" />
					</td>
					<td>
						<input style="width: 70px" type="button" onclick="SelectPrepMsg('/GuestAccess.aspx?ID=<%# Eval("ArticleID") %>&Type=<%=((int)Tools.MyVar.SiteGuest.Article)%>',520,490);"
