<%@ Page Title="" Language="C#" MasterPageFile="~/Admin.Master" AutoEventWireup="true" CodeBehind="ProductCategory.aspx.cs" Inherits="Portal.Admin.ProductCategory" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Body" runat="server">
	<table dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" class="TableColor" align="center" id="Table1" cellspacing="0" cellpadding="1"
		...
		<tr>
			<td colspan="2" class="RowHead">
				<%=GetGlobalResourceObject("resource", "StoreSubjectMng")%>
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			...
			<td>
				<asp:DropDownList ID="LeveLDL" runat="server" DataTextField="name" 
					DataValueField="level" AutoPostBack="false" >
				</asp:DropDownList>
				&nbsp;
				<asp:Button ID="SubCatAddBTN" runat="server" Text="اضافه شدن اتوماتیک شاخه بندی به محصولات" OnClick="SubCatAddBTN_Click" />
			</td>
		</tr>
		<tr>
			<td>
   ...
   {%>
	<table dir="<%=GetGlobalResourceObject("resource", "_Dir")%>" id="My" align="center" cellspacing="0" cellpadding="2" width="100%"
		...
		<asp:Repeater runat="server" ID="ViewDR" OnItemCommand="ViewDR_ItemCommand">
			<ItemTemplate>
				...
				<td align="center">
						<asp:ImageButton ImageUrl="Imgs/Up.png" Width="18px" Height="18px" CommandArgument='<%# Eval("Sort") +"#"+ Eval("ProductCategoryid") %>'
							CommandName="UP" ID="UpIB" ToolTip="<%$ resources: resource, MoveUp %>" AlternateText="<%$ resources: resource, MoveUp %>"
							runat="server" />
						<asp:ImageButton ImageUrl="Imgs/down.png" Width="18px" Height="18px" CommandArgument='<%# Eval("Sort") +"#"+ Eval("ProductCategoryid") %>'
							CommandName="DOWN" ID="DownBTN" ToolTip="<%$ resources: resource, MoveDown %>" AlternateText="<%$ resources: resource, MoveDown %>"
							runat="server" />
					</td>
					<td align="center">
						&nbsp;<%# Eval("ProductCategoryid").ToString().Trim()%></td>
					<td align="right">
						&nbsp;<%#Tools.Tools.GetLevelTab(Eval("[level]").ToString().Trim())%><%#Eval("Name").ToString().Trim()%></td>
					<td>
						<%# GetProductCNT( Eval("ProductCategoryid").ToString().Trim())%>
					</td>
					<td align="center">
						<asp:ImageButton ImageUrl="Imgs/edit.gif" Width="18px" Height="18px" CommandArgument='<%# Eval("ProductCategoryid").ToString().Trim() %>'
					...
					<td align="center">
						<asp:ImageButton OnClientClick="return confirm_delete();" ImageUrl="Imgs/zarb.gif"
							Width="16px" Height="16px" CommandArgument='<%# Eval("ProductCategoryid").ToString().Trim() %>'
	...
	<%}%><asp:Button ID="DELBTN" runat="server" Text="حذف تمامی موضوعات" OnClick="DELBTN_Click" />
</asp:Content>
